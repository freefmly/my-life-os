...
function makeInvestmentChart(records, series, legend) {
  ...
  const width = 640; const height = 220; const padding = { top: 16, right: 12, bottom: 30, left: 12 };
  ...
  const primaryDots = records.map((record, index) => { const coord = point(record[series[0].key], index); return `<circle class="performance-dot" cx="${coord.x.toFixed(1)}" cy="${coord.y.toFixed(1)}" r="3.5"><title>${investmentDate(record.date)} · ${formatInvestmentMoney(investmentNumber(record[series[0].key]))}</title></circle>`; }).join('');
  const grid = [0, .5, 1].map((ratio) => { const y = padding.top + (height - padding.top - padding.bottom) * ratio; return `<line class="performance-grid-line" x1="${padding.left}" x2="${width - padding.right}" y1="${y}" y2="${y}"/>`; }).join('');
  return `<svg class="performance-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="투자 성과 추이 그래프">${grid}${lines}${primaryDots}<text x="${padding.left}" y="${height - 7}" fill="#788078" font-size="11">${investmentDate(records[0].date)}</text><text x="${width - padding.right}" y="${height - 7}" text-anchor="end" fill="#788078" font-size="11">${investmentDate(records.at(-1).date)}</text></svg><div class="chart-legend">${legend.map((item) => `<span class="${item.className || ''}">${item.label}</span>`).join('')}</div>`;
}
function snapshotSoxl(date = new Date().toISOString()) { const soxl = state.investments.soxl; const day = String(date).slice(0, 10); const snapshot = { id: createGoalId(), date, valuation: investmentNumber(soxl.valuation), cash: investmentNumber(soxl.cash), total: investmentNumber(soxl.valuation) + investmentNumber(soxl.cash) }; soxl.snapshots = soxl.snapshots.filter((item) => String(item.date).slice(0, 10) !== day); soxl.snapshots.push(snapshot); }
...
function renderInvestments() {
  ...
  renderFamilyAccount();
  const soxlCurrentPrice = marketQuotePrice('SOXL', investmentNumber(soxl.shares) ? investmentNumber(soxl.valuation) / investmentNumber(soxl.shares) : 0);
  const soxlValuation = investmentNumber(soxl.shares) ? investmentNumber(soxl.shares) * soxlCurrentPrice : investmentNumber(soxl.valuation);
  const accountValue = soxlValuation + investmentNumber(soxl.cash);
  const grossProfit = accountValue - investmentNumber(soxl.principal);
  const subscriptionFeeUsd = investmentNumber(soxl.subscriptionFeeUsd);
  const netProfit = grossProfit - subscriptionFeeUsd;
  const includeFee = soxl.includeFee !== false;
  const profit = includeFee ? netProfit : grossProfit;
  ...
  soxlTradeList.innerHTML = soxl.trades.length ? [...soxl.trades].sort((a, b) => new Date(b.date) - new Date(a.date)).map((trade) => `<article class="investment-record"><time>${investmentDate(trade.date)}</time><div><strong>${trade.type === 'buy' ? 'SOXL 매수' : 'SOXL 매도'} · ${formatNumber(trade.shares)}주 · ${formatInvestmentMoney(trade.price)}</strong><p>${escapeHtml(trade.note || '메모 없음')}</p></div><span class="trade-badge ${trade.type === 'sell' ? 'sell' : ''}">${trade.type === 'buy' ? '매수' : '매도'}</span><span class="record-amount">${formatInvestmentMoney(trade.amount)}</span></article>`).join('') : '<div class="investment-empty">첫 매수·매도 기록을 남겨보세요.</div>';
  ...
  tqqqSummary.innerHTML = [investmentMetric('다음 V', formatInvestmentMoney(current.targetValue)), investmentMetric('현재 평가금 E', formatInvestmentMoney(position.valuation), 'accent'), investmentMetric('Pool · 예수금', formatInvestmentMoney(position.pool)), investmentMetric('G값', formatNumber(current.gValue)), investmentMetric('현재 TQQQ 가격', formatInvestmentMoney(position.currentPrice)), investmentMetric('보유 수량', `${formatNumber(position.shares)}주`), investmentMetric('평균 매수가', formatInvestmentMoney(position.averagePrice)), investmentMetric(modeLabel, modeValue), investmentMetric('하단 밴드', formatInvestmentMoney(current.lowerBand)), investmentMetric('상단 밴드', formatInvestmentMoney(current.upperBand)), investmentMetric('E와 V의 차이', `${gap >= 0 ? '+' : ''}${formatInvestmentMoney(gap)} · ${gapRate >= 0 ? '+' : ''}${formatPercent(gapRate)}`, 'dark', gap < 0 ? 'negative' : 'positive')].join('');
  const cycleChartRecords = cycles.map((cycle) => ({ ...cycle, valuation: calculateVrPosition(cycle).valuation }));
  tqqqChart.innerHTML = makeInvestmentChart(cycleChartRecords, [{ key: 'valuation' }, { key: 'targetValue', className: 'is-target' }, { key: 'lowerBand', className: 'is-lower' }, { key: 'upperBand', className: 'is-upper' }], [{ label: '평가금 E' }, { label: '다음 V', className: 'legend-target' }, { label: '상·하단 밴드', className: 'legend-band' }]);
  $('#tqqqChartCaption').textContent = `${cycles.length}개 사이클 · 최근 ${investmentDate(current.date)}`;
...
function createDefaultInvestments() { return { soxl: { principal: 0, purchaseAmount: 0, cash: 0, shares: 0, averagePrice: 0, valuation: 0, subscriptionFeeKrw: 0, subscriptionFeeUsd: 0, includeFee: true, trades: [], snapshots: [] }, tqqqVr: { mode: 'installment', contribution: 0, withdrawal: 0, cycles: [], trades: [] } }; }
...
function openSoxlSnapshotDialog() { const soxl = state.investments.soxl; [['soxlPrincipal', 'principal'], ['soxlPurchaseAmount', 'purchaseAmount'], ['soxlCash', 'cash'], ['soxlShares', 'shares'], ['soxlAveragePrice', 'averagePrice'], ['soxlValuation', 'valuation'], ['soxlSubscriptionFee', 'subscriptionFeeKrw']].forEach(([input, key]) => { $(`#${input}`).value = soxl[key] || ''; }); $('#soxlSnapshotDialog').showModal(); $('#soxlPrincipal').focus(); }
function openSoxlTradeDialog() { $('#soxlTradeForm').reset(); $('#soxlTradeDate').value = new Date().toISOString().slice(0, 10); $('#soxlTradeDialog').showModal(); $('#soxlTradeShares').focus(); }
...
function openVrPriceDialog() { const cycle = currentVrCycle(); if (!cycle) { alert('먼저 새 리밸런싱 사이클을 시작해주세요.'); return; } $('#vrPriceForm').reset(); $('#vrPriceDate').value = new Date().toISOString().slice(0, 10); $('#vrPriceValue').value = calculateVrPosition(cycle).currentPrice || ''; $('#vrPriceDialog').showModal(); $('#vrPriceValue').focus(); }
...
$('#updateVrPriceButton').addEventListener('click', openVrPriceDialog);
...
$('#soxlSnapshotForm').addEventListener('submit', async (event) => { event.preventDefault(); const soxl = state.investments.soxl; const subscriptionFeeKrw = investmentNumber($('#soxlSubscriptionFee').value); try { const rate = subscriptionFeeKrw ? await loadUsdKrwRate() : 1; [['principal', 'soxlPrincipal'], ['purchaseAmount', 'soxlPurchaseAmount'], ['cash', 'soxlCash'], ['shares', 'soxlShares'], ['averagePrice', 'soxlAveragePrice'], ['valuation', 'soxlValuation']].forEach(([key, input]) => { soxl[key] = investmentNumber($(`#${input}`).value); }); soxl.subscriptionFeeKrw = subscriptionFeeKrw; soxl.subscriptionFeeUsd = subscriptionFeeKrw ? subscriptionFeeKrw / rate : 0; snapshotSoxl(); persist(); render(); $('#soxlSnapshotDialog').close(); } catch { alert('구독료 환산을 위한 환율을 불러오지 못했어요. 인터넷 연결을 확인한 뒤 다시 시도해주세요.'); } });
$('#soxlTradeForm').addEventListener('submit', (event) => { event.preventDefault(); const soxl = state.investments.soxl; const type = $('#soxlTradeType').value; const shares = investmentNumber($('#soxlTradeShares').value); const price = investmentNumber($('#soxlTradePrice').value); const amount = shares * price; if (!shares || !price) return; const previousShares = investmentNumber(soxl.shares); if (type === 'buy') { soxl.averagePrice = previousShares + shares ? ((previousShares * investmentNumber(soxl.averagePrice)) + amount) / (previousShares + shares) : 0; soxl.shares = previousShares + shares; soxl.cash = investmentNumber(soxl.cash) - amount; soxl.purchaseAmount = investmentNumber(soxl.purchaseAmount) + amount; } else { if (shares > previousShares) { alert('현재 보유 수량보다 많이 매도할 수 없어요.'); return; } soxl.shares = previousShares - shares; soxl.cash = investmentNumber(soxl.cash) + amount; if (!soxl.shares) soxl.averagePrice = 0; } soxl.valuation = investmentNumber(soxl.shares) * price; soxl.trades.unshift({ id: createGoalId(), date: $('#soxlTradeDate').value, type, shares, price, amount, note: $('#soxlTradeNote').value.trim() }); snapshotSoxl(`${$('#soxlTradeDate').value}T12:00:00`); persist(); render(); $('#soxlTradeDialog').close(); });
$('#vrCycleForm').addEventListener('submit', (event) => { event.preventDefault(); const vr = state.investments.tqqqVr; const mode = vr.mode || 'installment'; const flowAmount = mode === 'lump-sum' ? 0 : investmentNumber($('#vrFlowAmount').value); if (mode === 'installment') vr.contribution = flowAmount; if (mode === 'withdrawal') vr.withdrawal = flowAmount; const existing = editingVrCycleId ? vr.cycles.find((item) => item.id === editingVrCycleId) : null; const cycle = { id: existing?.id || createGoalId(), date: $('#vrCycleDate').value, targetValue: investmentNumber($('#vrTargetValue').value), gValue: investmentNumber($('#vrGValue').value), lowerBand: investmentNumber($('#vrLowerBand').value), upperBand: investmentNumber($('#vrUpperBand').value), pool: investmentNumber($('#vrPool').value), shares: investmentNumber($('#vrShares').value), averagePrice: investmentNumber($('#vrAveragePrice').value), currentPrice: investmentNumber($('#vrCurrentPrice').value), flowAmount, note: $('#vrCycleNote').value.trim() }; if (existing) vr.cycles[vr.cycles.findIndex((item) => item.id === existing.id)] = cycle; else vr.cycles.push(cycle); persist(); render(); $('#vrCycleDialog').close(); });
$('#vrTradeForm').addEventListener('submit', (event) => { event.preventDefault(); const cycle = currentVrCycle(); if (!cycle) return; const type = $('#vrTradeType').value; const shares = investmentNumber($('#vrTradeShares').value); const price = investmentNumber($('#vrTradePrice').value); if (!shares || !price) return; const position = calculateVrPosition(cycle); if (type === 'sell' && shares > position.shares) { alert('현재 보유 수량보다 많이 매도할 수 없어요.'); return; } cycle.currentPrice = price; cycle.priceUpdatedAt = $('#vrTradeDate').value; state.investments.tqqqVr.trades.push({ id: createGoalId(), cycleId: cycle.id, date: $('#vrTradeDate').value, type, shares, price, note: $('#vrTradeNote').value.trim() }); persist(); render(); $('#vrTradeDialog').close(); });
$('#vrPriceForm').addEventListener('submit', (event) => { event.preventDefault(); const cycle = currentVrCycle(); if (!cycle) return; cycle.currentPrice = investmentNumber($('#vrPriceValue').value); cycle.priceUpdatedAt = $('#vrPriceDate').value; persist(); render(); $('#vrPriceDialog').close(); });
