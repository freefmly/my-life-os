...
function snapshotSoxl(date = new Date().toISOString()) { const soxl = state.investments.soxl; soxl.snapshots.push({ id: createGoalId(), date, valuation: investmentNumber(soxl.valuation), cash: investmentNumber(soxl.cash), total: investmentNumber(soxl.valuation) + investmentNumber(soxl.cash) }); }
...
function renderInvestments() {
  ...
  $('#exchangeRateCaption').textContent = investmentDisplayCurrency === 'KRW' && usdKrwRate ? `환율 1 USD = ${formatNumber(usdKrwRate)}원 · ${usdKrwRateDate}` : 'USD 기준';
  $('#toggleSoxlFeeButton').textContent = `구독료 반영: ${soxl.includeFee !== false ? '켜짐' : '꺼짐'}`;
  const accountValue = investmentNumber(soxl.valuation) + investmentNumber(soxl.cash);
  ...
  const position = calculateVrPosition(current); const gap = position.valuation - investmentNumber(current.targetValue); const gapRate = investmentNumber(current.targetValue) ? (gap / investmentNumber(current.targetValue)) * 100 : 0;
...
async function loadUsdKrwRate() { const response = await fetch('https://api.frankfurter.dev/v2/rate/USD/KRW'); if (!response.ok) throw new Error('rate fetch failed'); const data = await response.json(); const rate = Number(data.rate); if (!rate) throw new Error('invalid rate'); usdKrwRate = rate; usdKrwRateDate = data.date || ''; return rate; }
...
$('#toggleInvestmentCurrency').addEventListener('click', async (event) => { if (investmentDisplayCurrency === 'KRW') { investmentDisplayCurrency = 'USD'; renderInvestments(); return; } event.currentTarget.textContent = '환율 불러오는 중…'; event.currentTarget.disabled = true; try { await loadUsdKrwRate(); investmentDisplayCurrency = 'KRW'; } catch { alert('환율을 불러오지 못했어요. 잠시 후 다시 시도해주세요.'); } finally { event.currentTarget.disabled = false; renderInvestments(); } });
