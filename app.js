...
function renderInvestments() {
  ...
  if (!current) { tqqqSummary.innerHTML = [investmentMetric('다음 V', '—'), investmentMetric('현재 평가금 E', '—', 'accent'), investmentMetric('Pool · 예수금', '—'), investmentMetric('G값', '—')].join(''); tqqqChart.innerHTML = makeInvestmentChart([], [], []); $('#tqqqChartCaption').textContent = '첫 사이클을 기록해주세요'; tqqqCycleList.innerHTML = '<div class="investment-empty">실력공식/시트에서 계산한 첫 V값을 기록해보세요.</div>'; return; }
  const gap = investmentNumber(current.valuation) - investmentNumber(current.targetValue); const gapRate = investmentNumber(current.targetValue) ? (gap / investmentNumber(current.targetValue)) * 100 : 0;
  ...
  tqqqSummary.innerHTML = [investmentMetric('다음 V', formatInvestmentMoney(current.targetValue)), investmentMetric('현재 평가금 E', formatInvestmentMoney(current.valuation), 'accent'), investmentMetric('Pool · 예수금', formatInvestmentMoney(current.pool)), investmentMetric('G값', formatNumber(current.gValue)), investmentMetric('현재 TQQQ 가격', formatInvestmentMoney(current.currentPrice)), investmentMetric('보유 수량', `${formatNumber(investmentNumber(current.shares))}주`), investmentMetric('평균 매수가', formatInvestmentMoney(current.averagePrice)), investmentMetric(modeLabel, modeValue), investmentMetric('하단 밴드', formatInvestmentMoney(current.lowerBand)), investmentMetric('상단 밴드', formatInvestmentMoney(current.upperBand)), investmentMetric('E와 V의 차이', `${gap >= 0 ? '+' : ''}${formatInvestmentMoney(gap)} · ${gapRate >= 0 ? '+' : ''}${formatPercent(gapRate)}`, 'dark', gap < 0 ? 'negative' : 'positive')].join('');
  tqqqChart.innerHTML = makeInvestmentChart(cycles, [{ key: 'valuation' }, { key: 'targetValue', className: 'is-target' }, { key: 'lowerBand', className: 'is-lower' }, { key: 'upperBand', className: 'is-upper' }], [{ label: '평가금 E' }, { label: '다음 V', className: 'legend-target' }, { label: '상·하단 밴드', className: 'legend-band' }]);
  $('#tqqqChartCaption').textContent = `${cycles.length}개 사이클 · 최근 ${investmentDate(current.date)}`;
  tqqqCycleList.innerHTML = [...cycles].reverse().map((cycle) => `<article class="investment-record"><time>${investmentDate(cycle.date)}</time><div><strong>다음 V ${formatInvestmentMoney(cycle.targetValue)} · E ${formatInvestmentMoney(cycle.valuation)} · G ${formatNumber(cycle.gValue)}</strong><p>${escapeHtml(cycle.note || '실행 메모 없음')}</p></div><span class="trade-badge">Pool ${formatInvestmentMoney(cycle.pool)}</span><span class="record-amount">${formatNumber(cycle.shares)}주</span></article>`).join('');
}
...
function createDefaultInvestments() { return { soxl: { principal: 0, purchaseAmount: 0, cash: 0, shares: 0, averagePrice: 0, valuation: 0, subscriptionFee: 0, includeFee: true, trades: [], snapshots: [] }, tqqqVr: { mode: 'installment', contribution: 0, withdrawal: 0, cycles: [] } }; }
...
function openVrCycleDialog() { const vr = state.investments.tqqqVr; const current = sortedRecords(vr.cycles).at(-1); $('#vrCycleForm').reset(); $('#vrCycleDate').value = new Date().toISOString().slice(0, 10); if (current) [['vrTargetValue', 'targetValue'], ['vrGValue', 'gValue'], ['vrLowerBand', 'lowerBand'], ['vrUpperBand', 'upperBand'], ['vrPool', 'pool'], ['vrShares', 'shares'], ['vrAveragePrice', 'averagePrice'], ['vrCurrentPrice', 'currentPrice'], ['vrValuation', 'valuation']].forEach(([input, key]) => { $(`#${input}`).value = current[key] || ''; }); $('#vrFlowAmount').value = vr.mode === 'withdrawal' ? vr.withdrawal || '' : vr.contribution || ''; syncVrFlowField(); $('#vrCycleDialog').showModal(); $('#vrTargetValue').focus(); }
...
$('#toggleInvestmentCurrency').addEventListener('click', async (event) => { if (investmentDisplayCurrency === 'KRW') { investmentDisplayCurrency = 'USD'; renderInvestments(); return; } event.currentTarget.textContent = '환율 불러오는 중…'; event.currentTarget.disabled = true; try { const response = await fetch('https://api.frankfurter.dev/v2/rate/USD/KRW'); if (!response.ok) throw new Error('rate fetch failed'); const data = await response.json(); usdKrwRate = Number(data.rate); usdKrwRateDate = data.date || ''; if (!usdKrwRate) throw new Error('invalid rate'); investmentDisplayCurrency = 'KRW'; } catch { alert('환율을 불러오지 못했어요. 잠시 후 다시 시도해주세요.'); } finally { event.currentTarget.disabled = false; renderInvestments(); } });
...
$('#soxlSnapshotForm').addEventListener('submit', (event) => { event.preventDefault(); const soxl = state.investments.soxl; [['principal', 'soxlPrincipal'], ['purchaseAmount', 'soxlPurchaseAmount'], ['cash', 'soxlCash'], ['shares', 'soxlShares'], ['averagePrice', 'soxlAveragePrice'], ['valuation', 'soxlValuation'], ['subscriptionFee', 'soxlSubscriptionFee']].forEach(([key, input]) => { soxl[key] = investmentNumber($(`#${input}`).value); }); snapshotSoxl(); persist(); render(); $('#soxlSnapshotDialog').close(); });
...
$('#vrCycleForm').addEventListener('submit', (event) => { event.preventDefault(); const vr = state.investments.tqqqVr; const mode = vr.mode || 'installment'; const flowAmount = mode === 'lump-sum' ? 0 : investmentNumber($('#vrFlowAmount').value); if (mode === 'installment') vr.contribution = flowAmount; if (mode === 'withdrawal') vr.withdrawal = flowAmount; const cycle = { id: createGoalId(), date: $('#vrCycleDate').value, targetValue: investmentNumber($('#vrTargetValue').value), gValue: investmentNumber($('#vrGValue').value), lowerBand: investmentNumber($('#vrLowerBand').value), upperBand: investmentNumber($('#vrUpperBand').value), pool: investmentNumber($('#vrPool').value), shares: investmentNumber($('#vrShares').value), averagePrice: investmentNumber($('#vrAveragePrice').value), currentPrice: investmentNumber($('#vrCurrentPrice').value), valuation: investmentNumber($('#vrValuation').value), flowAmount, note: $('#vrCycleNote').value.trim() }; vr.cycles.push(cycle); persist(); render(); $('#vrCycleDialog').close(); });
