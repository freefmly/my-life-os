...
function renderGrowth() {
  ...
  const logRow = (log, timeFormat) => `<div class="growth-log"><span>✦</span><div><strong>${escapeHtml(log.title)}</strong><p>${rewardSummary(log.rewards)} · 캐릭터 +${log.lifeXp} XP</p></div><time>${new Intl.DateTimeFormat('ko-KR', timeFormat).format(new Date(log.loggedAt))}</time><button class="delete-growth-log" type="button" data-delete-growth-log="${log.id}" title="성장 기록 삭제" aria-label="${escapeHtml(log.title)} 성장 기록 삭제">×</button></div>`;
  growthLogList.innerHTML = state.growth.logs.length ? state.growth.logs.slice(0, 6).map((log) => logRow(log, { month: 'short', day: 'numeric' })).join('') : '<div class="growth-empty">첫 활동을 기록하면 성장 이야기가 쌓여요.</div>';
  const todayKey = new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Seoul' });
  const todayLogs = state.growth.logs.filter((log) => new Date(log.loggedAt).toLocaleDateString('sv-SE', { timeZone: 'Asia/Seoul' }) === todayKey);
  todayActivityLogList.innerHTML = todayLogs.length ? todayLogs.map((log) => logRow(log, { hour: 'numeric', minute: 'numeric' })).join('') : '<div class="growth-empty">오늘 기록한 활동이 없어요. 작은 행동 하나를 기록해볼까요?</div>';
