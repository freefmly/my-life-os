...
function formatInvestmentAxis(value) { const amount = investmentDisplayCurrency === 'KRW' && usdKrwRate ? investmentNumber(value) * usdKrwRate : investmentNumber(value); const prefix = investmentDisplayCurrency === 'KRW' && usdKrwRate ? '₩' : '$'; return `${prefix}${new Intl.NumberFormat('ko-KR', { notation: 'compact', maximumFractionDigits: 1 }).format(amount)}`; }
function makeInvestmentChart(records, series, legend) {
  if (records.length < 2) return '<div class="investment-empty">기록이 2개 이상 쌓이면<br>성과 추이 그래프를 보여드려요.</div>';
  const values = records.flatMap((record) => series.map((item) => investmentNumber(record[item.key])));
  const min = Math.min(...values); const max = Math.max(...values); const range = Math.max(max - min, Math.max(max, 1) * .1, 1);
  const width = 640; const height = 220; const padding = { top: 16, right: 12, bottom: 30, left: 76 };
  const point = (value, index) => ({ x: padding.left + ((width - padding.left - padding.right) * index) / (records.length - 1), y: padding.top + (height - padding.top - padding.bottom) * (1 - (investmentNumber(value) - min) / range) });
  const lines = series.map((item) => `<polyline class="performance-line ${item.className || ''}" points="${records.map((record, index) => { const coord = point(record[item.key], index); return `${coord.x.toFixed(1)},${coord.y.toFixed(1)}`; }).join(' ')}"/>`).join('');
  const primaryDots = records.map((record, index) => { const coord = point(record[series[0].key], index); const tooltip = `<strong>${investmentDate(record.date)}</strong>${series.map((item, seriesIndex) => `<span>${escapeHtml(legend[seriesIndex]?.label || item.key)} <b>${formatInvestmentMoney(investmentNumber(record[item.key]))}</b></span>`).join('')}`; return `<circle class="performance-dot" cx="${coord.x.toFixed(1)}" cy="${coord.y.toFixed(1)}" r="4" tabindex="0" role="button" aria-label="${investmentDate(record.date)} 투자 성과 자세히 보기" data-chart-tooltip="${encodeURIComponent(tooltip)}"/>`; }).join('');
  const grid = [0, .5, 1].map((ratio) => { const y = padding.top + (height - padding.top - padding.bottom) * ratio; const value = min + range * (1 - ratio); return `<line class="performance-grid-line" x1="${padding.left}" x2="${width - padding.right}" y1="${y}" y2="${y}"/><text class="performance-axis-label" x="${padding.left - 9}" y="${y + 4}" text-anchor="end">${formatInvestmentAxis(value)}</text>`; }).join('');
  return `<svg class="performance-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="투자 성과 추이 그래프">${grid}${lines}${primaryDots}<text class="performance-date-label" x="${padding.left}" y="${height - 7}">${investmentDate(records[0].date)}</text><text class="performance-date-label" x="${width - padding.right}" y="${height - 7}" text-anchor="end">${investmentDate(records.at(-1).date)}</text></svg><div class="investment-chart-tooltip" role="status" aria-live="polite"></div><div class="chart-legend">${legend.map((item) => `<span class="${item.className || ''}">${item.label}</span>`).join('')}</div>`;
}
...
function soxlPosition() { const soxl = state.investments.soxl; const principal = investmentNumber(soxl.principal); const shares = investmentNumber(soxl.shares); const averagePrice = investmentNumber(soxl.averagePrice); const purchaseAmount = shares * averagePrice; const fallbackPrice = shares ? investmentNumber(soxl.valuation) / shares : 0; const currentPrice = marketQuotePrice('SOXL', fallbackPrice); const cash = principal - purchaseAmount; const valuation = shares * currentPrice; const accountValue = valuation + cash; const subscriptionFeeUsd = soxl.includeFee === false ? 0 : investmentNumber(soxl.subscriptionFeeUsd); const profit = accountValue - principal - subscriptionFeeUsd; return { principal, shares, averagePrice, purchaseAmount, currentPrice, cash, valuation, accountValue, profit, returnRate: principal + subscriptionFeeUsd ? (profit / (principal + subscriptionFeeUsd)) * 100 : 0 }; }
...
function renderInvestments() {
  ...
  const soxlRecords = dailySoxlSnapshots(soxl.snapshots);
  ...
  const legacyCycleRecords = cycles.map((cycle) => ({ ...cycle, valuation: calculateVrPosition(cycle).valuation, total: calculateVrPosition(cycle).valuation + calculateVrPosition(cycle).pool }));
  const cycleChartRecords = chartRecordsForRange(vr.snapshots.length ? vr.snapshots : legacyCycleRecords, tqqqChartRange);
  tqqqChart.innerHTML = makeInvestmentChart(cycleChartRecords, [{ key: 'total' }, { key: 'targetValue', className: 'is-target' }, { key: 'lowerBand', className: 'is-lower' }, { key: 'upperBand', className: 'is-upper' }], [{ label: '총 자산' }, { label: '다음 V', className: 'legend-target' }, { label: '상·하단 밴드', className: 'legend-band' }]);
  ...
  vrTradeList.innerHTML = currentTrades.length ? currentTrades.map((trade) => `<article class="investment-record"><time>${investmentDate(trade.date)}</time><div><strong>${trade.type === 'buy' ? 'TQQQ 매수' : 'TQQQ 매도'} · ${formatNumber(trade.shares)}주 · ${formatInvestmentMoney(trade.price)}</strong><p>${escapeHtml(trade.note || '메모 없음')}</p></div><span class="trade-badge ${trade.type === 'sell' ? 'sell' : ''}">${trade.type === 'buy' ? '매수' : '매도'}</span><span class="record-amount">${formatInvestmentMoney(investmentNumber(trade.shares) * investmentNumber(trade.price))}</span></article>`).join('') : '<div class="investment-empty">이번 사이클에 기록된 리밸런싱 거래가 없어요.</div>';
...
function openLoreDialog(loreId = null) { const content = activeContent(); if (!content) return; editingLoreId = loreId; const item = loreId ? content.lore.find((lore) => lore.id === loreId) : null; editingLoreParentIds = item ? loreParentIds(item) : []; $('#loreForm').reset(); $('#loreDialogEyebrow').textContent = item ? 'EDIT WORLD NOTE' : 'NEW WORLD NOTE'; $('#loreDialogTitle').textContent = item ? '세계관 설정을 다듬어보세요.' : '세계관 설정을 추가해요.'; populateLoreParents(loreId); renderLoreParentList(); if (item) { $('#loreName').value = item.name; $('#loreType').value = item.type; $('#loreNote').value = item.note || ''; if (/^https?:\/\//.test(item.image || '')) $('#loreImageUrl').value = item.image; } $('#removeLoreImageOption').hidden = !item?.image; toggleLoreImageFields(); $('#loreDialog').showModal(); $('#loreName').focus(); }
...
function openSoxlTradeDialog(tradeId = null) { const trade = tradeId ? state.investments.soxl.trades.find((item) => item.id === tradeId) : null; editingSoxlTradeId = trade?.id || null; $('#soxlTradeForm').reset(); $('#soxlTradeDate').value = trade?.date || new Date().toISOString().slice(0, 10); $('#soxlTradeType').value = trade?.type || 'buy'; $('#soxlTradeShares').value = trade?.shares || ''; $('#soxlTradePrice').value = trade?.price || ''; $('#soxlTradeNote').value = trade?.note || ''; $('#soxlTradeDialog h2').textContent = trade ? '매매 기록 수정' : '매수·매도 기록'; $('#soxlTradeDialog .save-button').textContent = trade ? '수정 저장' : '거래 저장'; $('#soxlTradeDialog').showModal(); $('#soxlTradeShares').focus(); }
function applySoxlTrade(trade, direction = 1) { const soxl = state.investments.soxl; const shares = investmentNumber(trade.shares); const price = investmentNumber(trade.price); const previousShares = investmentNumber(soxl.shares); let nextShares = previousShares; let nextAveragePrice = investmentNumber(soxl.averagePrice); if (!shares || !price) return false; if (trade.type === 'buy') { if (direction > 0) { nextAveragePrice = (previousShares * nextAveragePrice + shares * price) / (previousShares + shares); nextShares = previousShares + shares; } else { if (shares > previousShares) return false; nextShares = previousShares - shares; nextAveragePrice = nextShares ? Math.max(0, ((previousShares * nextAveragePrice) - shares * price) / nextShares) : 0; } } else if (direction > 0) { if (shares > previousShares) return false; nextShares = previousShares - shares; if (!nextShares) nextAveragePrice = 0; } else { nextShares = previousShares + shares; if (!nextAveragePrice) nextAveragePrice = price; } const purchaseAmount = nextShares * nextAveragePrice; if (direction > 0 && trade.type === 'buy' && purchaseAmount > investmentNumber(soxl.principal)) { alert('예수금이 부족해요. 먼저 투자원금을 수정해주세요.'); return false; } soxl.shares = nextShares; soxl.averagePrice = nextAveragePrice; soxl.purchaseAmount = purchaseAmount; soxl.cash = investmentNumber(soxl.principal) - purchaseAmount; soxl.valuation = nextShares * marketQuotePrice('SOXL', price); return true; }
...
function openVrTradeDialog() { if (!currentVrCycle()) { alert('먼저 새 리밸런싱 사이클을 시작해주세요.'); return; } $('#vrTradeForm').reset(); $('#vrTradeDate').value = new Date().toISOString().slice(0, 10); $('#vrTradeDialog').showModal(); $('#vrTradeShares').focus(); }
...
$('#vrTradeForm').addEventListener('submit', (event) => { event.preventDefault(); const cycle = currentVrCycle(); if (!cycle) return; const type = $('#vrTradeType').value; const shares = investmentNumber($('#vrTradeShares').value); const price = investmentNumber($('#vrTradePrice').value); if (!shares || !price) return; const position = calculateVrPosition(cycle); if (type === 'sell' && shares > position.shares) { alert('현재 보유 수량보다 많이 매도할 수 없어요.'); return; } cycle.currentPrice = price; cycle.priceUpdatedAt = $('#vrTradeDate').value; const date = $('#vrTradeDate').value; state.investments.tqqqVr.trades.push({ id: createGoalId(), cycleId: cycle.id, date, type, shares, price, note: $('#vrTradeNote').value.trim() }); snapshotTqqqVr(`${date}T21:30:00.000Z`); persist(); render(); $('#vrTradeDialog').close(); });
...
// Memory albums and a deliberately small people directory. Both are stored in the existing synced state.
let editingMemoryId = null;
let editingPersonId = null;
let activeMemoryId = null;
let activeMemoryFilter = 'all';
let memoryDraftImages = [];
let memoryDraftCoordinates = null;
let contextMemoryImageIndex = null;
const memoryImageUrlCache = new Map();
let memoryImageRefreshPromise = null;
const newRecordId = () => globalThis.crypto?.randomUUID?.() || `record-${Date.now()}-${Math.random().toString(16).slice(2)}`;
function storagePathFromLegacySignedUrl(value) { if (typeof value !== 'string') return ''; const match = value.match(/\/storage\/v1\/object\/sign\/memory-images\/([^?]+)/); return match ? decodeURIComponent(match[1]) : ''; }
function normalizeMemoryStorageState() { let changed = false; state.memories = (state.memories || []).map((memory) => { const coverPath = storagePathFromLegacySignedUrl(memory.coverImage); if (coverPath) changed = true; return { ...memory, coverImage: coverPath || memory.coverImage || '', images: (memory.images || []).map((image) => { const value = typeof image === 'string' ? image : image?.path || image?.src || ''; const path = storagePathFromLegacySignedUrl(value); if (path) { changed = true; return path; } return value; }) }; }); return changed; }
const isLocalMemoryImage = (image) => typeof image === 'string' && image.startsWith('data:');
const memoryImageSource = (image) => isLocalMemoryImage(image) ? image : memoryImageUrlCache.get(image || '') || '';
async function refreshMemoryImageUrls() { if (!signedInUser || !supabaseClient || memoryImageRefreshPromise) return memoryImageRefreshPromise; const paths = [...new Set((state.memories || []).flatMap((memory) => memory.images || []).filter((image) => image && !isLocalMemoryImage(image) && !memoryImageUrlCache.has(image)))]; if (!paths.length) return; memoryImageRefreshPromise = Promise.all(paths.map(async (path) => { const { data, error } = await supabaseClient.storage.from('memory-images').createSignedUrl(path, 300); if (!error && data?.signedUrl) memoryImageUrlCache.set(path, data.signedUrl); })).then(() => { renderMemories(); if ($('#memoryDetailDialog').open && activeMemoryId) openMemoryDetail(activeMemoryId); }).catch((error) => console.warn('Memory image refresh failed', error)).finally(() => { memoryImageRefreshPromise = null; }); return memoryImageRefreshPromise; }
const dateLabel = (start, end) => { const format = (value) => value ? new Intl.DateTimeFormat('ko-KR', { year: 'numeric', month: 'long', day: 'numeric' }).format(new Date(`${value}T00:00:00`)) : ''; return end && end !== start ? `${format(start)} – ${format(end)}` : format(start); };
const memoryPeople = (memory) => state.people.filter((person) => memory.peopleIds.includes(person.id));
const memoryLocationLabel = (memory) => memory.location || (Number.isFinite(memory.latitude) && Number.isFinite(memory.longitude) ? `${memory.latitude.toFixed(4)}, ${memory.longitude.toFixed(4)}` : '');
function memoryMapMarkup(memory) { if (!Number.isFinite(memory.latitude) || !Number.isFinite(memory.longitude)) return ''; const lat = memory.latitude; const lng = memory.longitude; const delta = .012; const bbox = `${lng - delta},${lat - delta},${lng + delta},${lat + delta}`; const openUrl = `https://www.openstreetmap.org/?mlat=${encodeURIComponent(lat)}&mlon=${encodeURIComponent(lng)}#map=15/${encodeURIComponent(lat)}/${encodeURIComponent(lng)}`; return `<section class="memory-map"><div><span>⌖ ${escapeHtml(memoryLocationLabel(memory) || '기억의 장소')}</span><a href="${openUrl}" target="_blank" rel="noopener noreferrer">큰 지도 보기 ↗</a></div><iframe title="${escapeHtml(memory.title)} 위치 지도" src="https://www.openstreetmap.org/export/embed.html?bbox=${encodeURIComponent(bbox)}&layer=mapnik&marker=${encodeURIComponent(`${lat},${lng}`)}" loading="lazy"></iframe></section>`; }
function renderMemories() { const list = $('#memoryList'); const filters = $('#memoryFilters'); if (!list || !filters) return; refreshMemoryImageUrls(); const years = [...new Set(state.memories.map((memory) => (memory.date || '').slice(0, 4)).filter(Boolean))].sort().reverse(); if (activeMemoryFilter !== 'all' && !years.includes(activeMemoryFilter)) activeMemoryFilter = 'all'; filters.innerHTML = [`<button class="memory-filter ${activeMemoryFilter === 'all' ? 'is-active' : ''}" type="button" data-memory-filter="all">모든 추억</button>`, ...years.map((year) => `<button class="memory-filter ${activeMemoryFilter === year ? 'is-active' : ''}" type="button" data-memory-filter="${year}">${year}년</button>`)].join(''); const items = state.memories.filter((memory) => activeMemoryFilter === 'all' || memory.date?.startsWith(activeMemoryFilter)).sort((a, b) => (b.date || '').localeCompare(a.date || '')); list.innerHTML = items.length ? items.map((memory) => { const cover = memoryImageSource(memory.coverImage || memory.images[0]); const people = memoryPeople(memory); const location = memoryLocationLabel(memory); return `<article class="memory-card" data-open-memory="${memory.id}" tabindex="0" role="button"><div class="memory-card-cover ${cover ? '' : 'is-empty'}">${cover ? `<img src="${cover}" alt="${escapeHtml(memory.title)} 대표 사진" />` : '✦'}</div><div class="memory-card-body"><div class="memory-card-top"><span>${dateLabel(memory.date, memory.endDate)}</span><span>사진 ${memory.images.length}장</span></div><h3>${escapeHtml(memory.title)}</h3>${location ? `<span class="memory-location">⌖ ${escapeHtml(location)}</span>` : ''}${memory.story ? `<p>${escapeHtml(memory.story)}</p>` : '<p>아직 적어둔 이야기가 없어요.</p>'}<div>${memory.tags.slice(0, 3).map((tag) => `<span class="memory-tag">#${escapeHtml(tag)}</span>`).join('')}</div><div class="memory-card-foot"><span>${people.map((person) => escapeHtml(person.name)).join(' · ') || '나만의 순간'}</span><span>보기 →</span></div></div></article>`; }).join('') : '<div class="memory-empty">첫 번째 추억을 만들어보세요.<br />사진과 짧은 이야기만으로도 충분해요.</div>'; }
function renderPeople() { const list = $('#peopleList'); if (!list) return; list.innerHTML = state.people.length ? state.people.map((person) => { const count = state.memories.filter((memory) => memory.peopleIds.includes(person.id)).length; return `<article class="person-card" data-edit-person="${person.id}" tabindex="0" role="button"><span class="person-avatar">${escapeHtml(person.name.slice(0, 1))}</span><div><h3>${escapeHtml(person.name)}</h3><p>${escapeHtml(person.relation || person.note || '아직 적어둔 설명이 없어요.')}</p></div><span class="person-memory-count">추억 ${count}</span></article>`; }).join('') : '<div class="people-empty">가족, 친구, 동료처럼 소중한 사람부터 추가해보세요.</div>'; }
function renderMemoryPeopleOptions(selected = []) { const box = $('#memoryPeopleOptions'); box.innerHTML = state.people.length ? state.people.map((person) => `<label class="memory-person-option"><input type="checkbox" value="${person.id}" ${selected.includes(person.id) ? 'checked' : ''} />${escapeHtml(person.name)}${person.relation ? ` · ${escapeHtml(person.relation)}` : ''}</label>`).join('') : '<p class="upload-note">먼저 사람들 메뉴에서 인물을 추가하면 여기에서 연결할 수 있어요.</p>'; }
function showMemoryDraft() { const preview = $('#memoryImagePreview'); preview.innerHTML = memoryDraftImages.map((image, index) => `<img src="${image.preview}" alt="선택한 사진 미리보기 ${index + 1}" />`).join(''); }
function openMemoryDialog(id = null) { editingMemoryId = id; const memory = id ? state.memories.find((item) => item.id === id) : null; memoryDraftImages = []; memoryDraftCoordinates = Number.isFinite(memory?.latitude) && Number.isFinite(memory?.longitude) ? { latitude: memory.latitude, longitude: memory.longitude, locationQuery: memory.location || '' } : null; $('#memoryForm').reset(); $('#memoryDialogEyebrow').textContent = memory ? 'EDIT MEMORY' : 'NEW MEMORY'; $('#memoryDialogTitle').textContent = memory ? '추억을 다듬어보세요.' : '새로운 추억을 담아요.'; $('#memoryDate').value = memory?.date || new Date().toISOString().slice(0, 10); $('#memoryEndDate').value = memory?.endDate || ''; $('#memoryTitle').value = memory?.title || ''; $('#memoryLocation').value = memory?.location || ''; $('#memoryLocationStatus').textContent = memoryDraftCoordinates ? '지도 위치가 연결되어 있어요.' : ''; $('#memoryTags').value = memory?.tags?.join(', ') || ''; $('#memoryStory').value = memory?.story || ''; renderMemoryPeopleOptions(memory?.peopleIds || []); if (memory?.images?.length) memoryDraftImages = memory.images.map((src) => ({ preview: memoryImageSource(src), src, existing: true })); showMemoryDraft(); $('#memoryDialog').showModal(); $('#memoryTitle').focus(); }
async function imageDataUrl(file) { return new Promise((resolve) => { const reader = new FileReader(); reader.onload = () => resolve(reader.result); reader.readAsDataURL(file); }); }
async function uploadMemoryImage(file) { if (!signedInUser || !supabaseClient) return imageDataUrl(file); const { data: { session } } = await supabaseClient.auth.getSession(); if (!session?.access_token) throw new Error('로그인 세션을 찾지 못했어요. 로그아웃 후 다시 로그인해주세요.'); const ext = (file.name.split('.').pop() || 'jpg').replace(/[^a-z0-9]/gi, '').toLowerCase(); const path = `${signedInUser.id}/${newRecordId()}.${ext}`; const response = await fetch(`${SUPABASE_URL}/storage/v1/object/memory-images/${encodeURIComponent(path).replace(/%2F/g, '/')}`, { method: 'POST', headers: { apikey: SUPABASE_PUBLISHABLE_KEY, Authorization: `Bearer ${session.access_token}`, 'Content-Type': file.type, 'x-upsert': 'false' }, body: file }); if (!response.ok) { const detail = await response.text(); throw new Error(detail || `Storage 업로드 오류 (${response.status})`); } return path; }
function openMemoryDetail(id) { const memory = state.memories.find((item) => item.id === id); if (!memory) return; activeMemoryId = id; refreshMemoryImageUrls(); const people = memoryPeople(memory); $('#memoryDetailMeta').textContent = dateLabel(memory.date, memory.endDate); $('#memoryDetailTitle').textContent = memory.title; const photoMarkup = (image, index) => { const src = memoryImageSource(image); return src ? `<img src="${src}" alt="${escapeHtml(memory.title)} 사진 ${index + 1}" data-open-memory-image="${index}" />` : ''; }; const collage = memory.images.length ? `<div class="memory-collage">${memory.images.slice(0, 3).map(photoMarkup).join('')}</div>` : ''; const gallery = memory.images.length > 3 ? `<div class="memory-detail-gallery">${memory.images.slice(3).map((image, index) => photoMarkup(image, index + 3)).join('')}</div>` : ''; const locationOnly = memoryLocationLabel(memory) && !Number.isFinite(memory.latitude) ? `<p class="memory-detail-location">⌖ ${escapeHtml(memoryLocationLabel(memory))}</p>` : ''; $('#memoryDetailBody').innerHTML = `${collage}${locationOnly}${memoryMapMarkup(memory)}${memory.story ? `<p class="memory-detail-story">${escapeHtml(memory.story)}</p>` : '<p class="auth-intro">아직 적어둔 이야기가 없어요.</p>'}${memory.tags.length ? `<p>${memory.tags.map((tag) => `<span class="memory-tag">#${escapeHtml(tag)}</span>`).join('')}</p>` : ''}${people.length ? `<div class="memory-linked-people">${people.map((person) => `<button class="memory-linked-person" type="button" data-open-person="${person.id}">${escapeHtml(person.name)} →</button>`).join('')}</div>` : ''}${gallery}`; $('#memoryDetailDialog').showModal(); }
$('#addMemoryButton').addEventListener('click', () => openMemoryDialog());
$('#memoryImages').addEventListener('change', async (event) => { const files = [...event.target.files].filter((file) => file.type.startsWith('image/')); if (files.some((file) => file.size > 6 * 1024 * 1024)) { alert('사진 한 장은 6MB 이하로 선택해주세요.'); event.target.value = ''; return; } memoryDraftImages.push(...await Promise.all(files.map(async (file) => ({ file, preview: await imageDataUrl(file) })))); showMemoryDraft(); event.target.value = ''; });
async function geocodeMemoryLocation({ quiet = false } = {}) { const location = $('#memoryLocation').value.trim(); const status = $('#memoryLocationStatus'); if (!location) { memoryDraftCoordinates = null; status.textContent = ''; return false; } if (memoryDraftCoordinates?.locationQuery === location) return true; status.textContent = '장소를 지도에서 찾는 중…'; try { const response = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&accept-language=ko&q=${encodeURIComponent(location)}`); if (!response.ok) throw new Error('location search failed'); const results = await response.json(); if (!results.length) throw new Error('location not found'); memoryDraftCoordinates = { latitude: Number(results[0].lat), longitude: Number(results[0].lon), locationQuery: location }; status.textContent = '지도 위치를 연결했어요.'; return true; } catch (error) { console.warn(error); memoryDraftCoordinates = null; status.textContent = quiet ? '지도 위치를 찾지 못했어요.' : '장소를 찾지 못했어요. 더 구체적인 장소명으로 적어주세요.'; return false; } }
$('#memoryLocation').addEventListener('change', () => { geocodeMemoryLocation({ quiet: true }); });
$('#useCurrentLocationButton').addEventListener('click', () => { const status = $('#memoryLocationStatus'); if (!navigator.geolocation) { status.textContent = '이 브라우저는 위치 정보를 지원하지 않아요.'; return; } status.textContent = '현재 위치와 지역명을 확인 중…'; navigator.geolocation.getCurrentPosition(async (position) => { const latitude = position.coords.latitude; const longitude = position.coords.longitude; memoryDraftCoordinates = { latitude, longitude }; try { const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&accept-language=ko&lat=${encodeURIComponent(latitude)}&lon=${encodeURIComponent(longitude)}`); if (!response.ok) throw new Error('reverse geocoding failed'); const result = await response.json(); const address = result.address || {}; const parts = [address.neighbourhood || address.suburb || address.village || address.town || address.city_district, address.city || address.county || address.state].filter(Boolean); $('#memoryLocation').value = [...new Set(parts)].join(' · ') || result.display_name || ''; status.textContent = '현재 위치의 지역명을 담았어요.'; } catch (error) { console.warn(error); status.textContent = '지도 위치는 저장했어요. 장소명은 직접 적어주세요.'; } }, () => { status.textContent = '위치를 가져오지 못했어요. 권한을 확인해주세요.'; }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 300000 }); });
$('#memoryForm').addEventListener('submit', async (event) => { event.preventDefault(); const title = $('#memoryTitle').value.trim(); if (!title) return; const saveButton = $('#memoryForm .save-button'); saveButton.disabled = true; saveButton.textContent = '사진 저장 중…'; try { await geocodeMemoryLocation({ quiet: true }); const images = await Promise.all(memoryDraftImages.map((image) => image.existing ? image.src : uploadMemoryImage(image.file))); const previous = editingMemoryId ? state.memories.find((item) => item.id === editingMemoryId) : null; const memory = { id: previous?.id || newRecordId(), title, date: $('#memoryDate').value, endDate: $('#memoryEndDate').value, location: $('#memoryLocation').value.trim(), latitude: memoryDraftCoordinates?.latitude ?? null, longitude: memoryDraftCoordinates?.longitude ?? null, story: $('#memoryStory').value.trim(), tags: $('#memoryTags').value.split(',').map((tag) => tag.trim()).filter(Boolean).slice(0, 10), peopleIds: [...document.querySelectorAll('#memoryPeopleOptions input:checked')].map((input) => input.value), images, coverImage: previous?.coverImage && images.includes(previous.coverImage) ? previous.coverImage : images[0] || '', createdAt: previous?.createdAt || new Date().toISOString() }; if (previous) state.memories[state.memories.findIndex((item) => item.id === previous.id)] = memory; else state.memories.push(memory); persist(); render(); $('#memoryDialog').close(); } catch (error) { console.error(error); const message = String(error?.message || ''); const setupIssue = /bucket|row-level|permission|not authorized|policy/i.test(message); alert(setupIssue ? `사진 저장 권한이 아직 준비되지 않았어요. Supabase SQL Editor에서 supabase-schema.sql을 실행해주세요.\n\n오류: ${message}` : `사진을 저장하지 못했어요. 다시 시도해주세요.\n\n오류: ${message || '알 수 없음'}`); } finally { saveButton.disabled = false; saveButton.textContent = '추억 저장'; } });
$('#memoryFilters').addEventListener('click', (event) => { const button = event.target.closest('[data-memory-filter]'); if (!button) return; activeMemoryFilter = button.dataset.memoryFilter; renderMemories(); });
$('#memoryList').addEventListener('click', (event) => { const card = event.target.closest('[data-open-memory]'); if (card) openMemoryDetail(card.dataset.openMemory); });
$('#memoryList').addEventListener('keydown', (event) => { const card = event.target.closest('[data-open-memory]'); if (card && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); openMemoryDetail(card.dataset.openMemory); } });
$('#memoryDetailBody').addEventListener('click', (event) => { const image = event.target.closest('[data-open-memory-image]'); if (!image) return; $('#imageDialogPreview').src = image.src; $('#imageDialogPreview').alt = image.alt; imageDialog.showModal(); });
function hideMemoryPhotoContextMenu() { $('#memoryPhotoContextMenu').hidden = true; contextMemoryImageIndex = null; }
$('#memoryDetailDialog').addEventListener('close', () => { activeMemoryId = null; hideMemoryPhotoContextMenu(); });
$('#memoryDetailBody').addEventListener('contextmenu', (event) => { const image = event.target.closest('[data-open-memory-image]'); if (!image) { hideMemoryPhotoContextMenu(); return; } event.preventDefault(); contextMemoryImageIndex = Number(image.dataset.openMemoryImage); const menu = $('#memoryPhotoContextMenu'); menu.hidden = false; const maxLeft = window.innerWidth - menu.offsetWidth - 10; const maxTop = window.innerHeight - menu.offsetHeight - 10; menu.style.left = `${Math.max(10, Math.min(event.clientX, maxLeft))}px`; menu.style.top = `${Math.max(10, Math.min(event.clientY, maxTop))}px`; });
$('#setMemoryCoverButton').addEventListener('click', () => { const memory = state.memories.find((item) => item.id === activeMemoryId); const image = memory?.images[contextMemoryImageIndex]; if (!memory || !image) return hideMemoryPhotoContextMenu(); memory.coverImage = image; persist(); renderMemories(); hideMemoryPhotoContextMenu(); });
document.addEventListener('click', (event) => { if (!event.target.closest('#memoryPhotoContextMenu')) hideMemoryPhotoContextMenu(); });
window.addEventListener('resize', hideMemoryPhotoContextMenu);
$('#editMemoryButton').addEventListener('click', () => { const id = activeMemoryId; $('#memoryDetailDialog').close(); openMemoryDialog(id); });
async function deleteMemoryStorageFiles(memory) { const paths = memory.images.filter((image) => image && !isLocalMemoryImage(image)); if (!paths.length || !signedInUser || !supabaseClient) return; const { error } = await supabaseClient.storage.from('memory-images').remove(paths); if (error) throw error; paths.forEach((path) => memoryImageUrlCache.delete(path)); }
$('#deleteMemoryButton').addEventListener('click', async () => { const memory = state.memories.find((item) => item.id === activeMemoryId); if (!memory || !confirm(`“${memory.title}” 추억과 저장된 사진을 모두 삭제할까요?`)) return; const button = $('#deleteMemoryButton'); button.disabled = true; button.textContent = '사진 삭제 중…'; try { await deleteMemoryStorageFiles(memory); state.memories = state.memories.filter((item) => item.id !== activeMemoryId); persist(); render(); $('#memoryDetailDialog').close(); } catch (error) { console.error(error); alert('사진 원본을 삭제하지 못해 앨범 삭제를 취소했어요. 다시 시도해주세요.'); } finally { button.disabled = false; button.textContent = '삭제'; } });
['bookDialog', 'memoryDetailDialog', 'personDialog'].forEach((id) => { const dialogElement = $(`#${id}`); dialogElement.addEventListener('click', (event) => { if (event.target === dialogElement) dialogElement.close(); }); });
$('#addPersonButton').addEventListener('click', () => openPersonDialog());
function openPersonDialog(id = null) { editingPersonId = id; const person = id ? state.people.find((item) => item.id === id) : null; $('#personForm').reset(); $('#personDialogEyebrow').textContent = person ? 'EDIT PERSON' : 'NEW PERSON'; $('#personDialogTitle').textContent = person ? '사람 정보를 다듬어보세요.' : '소중한 사람을 기록해요.'; $('#personName').value = person?.name || ''; $('#personRelation').value = person?.relation || ''; $('#personNote').value = person?.note || ''; $('#personDialog').showModal(); $('#personName').focus(); }
$('#personForm').addEventListener('submit', (event) => { event.preventDefault(); const name = $('#personName').value.trim(); if (!name) return; const previous = editingPersonId ? state.people.find((item) => item.id === editingPersonId) : null; const person = { id: previous?.id || newRecordId(), name, relation: $('#personRelation').value.trim(), note: $('#personNote').value.trim() }; if (previous) state.people[state.people.findIndex((item) => item.id === previous.id)] = person; else state.people.push(person); persist(); render(); $('#personDialog').close(); });
$('#peopleList').addEventListener('click', (event) => { const card = event.target.closest('[data-edit-person]'); if (card) openPersonDialog(card.dataset.editPerson); });
$('#peopleList').addEventListener('keydown', (event) => { const card = event.target.closest('[data-edit-person]'); if (card && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); openPersonDialog(card.dataset.editPerson); } });
document.addEventListener('click', (event) => { const personLink = event.target.closest('[data-open-person]'); if (!personLink) return; $('#memoryDetailDialog').close(); setActiveView('people'); setTimeout(() => openPersonDialog(personLink.dataset.openPerson), 0); });
